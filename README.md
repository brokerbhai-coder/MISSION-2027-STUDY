...
| फ़ाइल / folder | काम |
|---|---|
| **`index.html`** | **मुख्य Homepage** — website यहीं से खुलती है |
| `css/style.css`, `css/responsive.css` | रंग और design |
| `js/app.js` | Home, Settings, बटनों का काम, शुरुआत |
| `js/router.js` | पेज बदलना (#/home, #/subject/physics ...) |
| `js/subjects.js` | विषयों की list और Subject Dashboard |
| `js/chapters.js` | Chapter की JSON पढ़ना, प्रश्नों की जाँच, Chapter lock/unlock |
| `js/quiz.js` | Quiz, Result, Review |
| `js/progress.js` | Score, accuracy, weak chapters, Progress पेज |
| `js/rewards.js` | XP, Level, Achievements, Daily Challenge |
| `js/mistakes.js` | Mistake Notebook |
| `js/timer.js` | पढ़ाई का असली समय |
| `js/games.js` | Brain Game (Unit Match) |
| `js/storage.js` | Progress सेव/लोड, Backup |
| **`data/subjects.json`** | **6 विषय और Chapters की list (यहीं से Chapter जोड़ते हैं)** |
| **`data/physics/chapter-01.json` … `chapter-08.json`** | **Physics के प्रश्न यहाँ जाएँगे (अभी खाली)** |
| `data/chemistry/ mathematics/ biology/ hindi/ english/` | बाकी विषयों के प्रश्न बाद में यहाँ आएँगे |
| `data/demo/demo-quiz.json` | सिर्फ़ जाँच के लिए 6 Demo सवाल (PDF वाले नहीं) |
| `data/question-template.json` | प्रश्न लिखने का नमूना (website इसे लोड नहीं करती) |
...
1. Chrome में **github.com** खोलो, login करो। Chrome के ⋮ मेन्यू में **"Desktop site"** ✔ करो (आसानी रहती है)।
2. **New repository** बनाओ (जैसे नाम: `mission2027`), **Public** रखो, **Create**।
3. Repository में **Settings → Pages** जाओ। *Source* में **Deploy from a branch** चुनो, Branch **main**, folder **/(root)** → **Save**।
4. 1–2 मिनट बाद वहीं ऊपर तुम्हारी website का लिंक दिखेगा: `https://तुम्हारा-नाम.github.io/mission2027/`
...
1. Repository में **Add file → Upload files** → Extract किए folder के **root की फ़ाइलें** चुनो: `index.html` और `README.md` → नीचे **Commit changes**।
2. फिर हर folder के लिए इस लिंक-पैटर्न से upload पेज खोलो (अपना नाम/repo बदलकर):
...
3. अगर कोई `upload/main/...` लिंक 404 दे, तो नीचे का तरीका B इस्तेमाल करो।
...
1. Pages का लिंक खोलो। (नया upload किया हो तो 1–2 मिनट रुको; पुराना पेज दिखे तो लिंक के आख़िर में `?v=2` जोड़ो या browser में refresh करो।)
2. Home पर **छह Subject cards** और "चलो Ankit, आज अपनी तैयारी शुरू करते हैं!" दिखना चाहिए।
3. **Physics** खोलो → 8 Chapters दिखें।
4. Home के नीचे **Demo Quiz** खेलो → Result में XP दिखे।
5. **सेटिंग → 🔍 Data Check** खोलो — यहाँ हर chapter के प्रश्नों की गिनती और कोई गलती दिखती है।
6. सफ़ेद/खाली पेज या "डेटा लोड नहीं हो पाया" दिखे तो नीचे *समस्या हो तो* देखो।
...
1. `data/subjects.json` खोलो, उस विषय के `"chapters"` list में एक नया block जोड़ो (Physics का Chapter 9 उदाहरण):
```json
{ "number": 9, "title": "Chapter का नाम", "file": "data/physics/chapter-09.json", "notesReady": true, "notesUrl": null }
```
   (पिछले block के बाद कॉमा `,` लगाना मत भूलना।)
2. `data/physics/chapter-09.json` नाम की नई फ़ाइल बनाओ (किसी पुरानी chapter फ़ाइल की copy करके):
```json
{ "subject": "physics", "chapter": 9, "title": "Chapter का नाम", "questions": [] }
```
3. उसमें प्रश्न डालो (ऊपर वाला format)।
4. Physics के अध्याय 9–15 के लिए `subjects.json` में `futureChapters` लिखा है — अध्याय जुड़ने पर उसे हटा सकते हो।
...
| कब | XP |
|---|---|
| नया सही उत्तर (हर प्रश्न का XP **सिर्फ़ पहली बार** सही करने पर) | 10 |
| Chapter का Quiz पहली बार पूरा | +20 |
| पहली बार पास (60%+) | +50 |
| Replay bonus (दिन में सिर्फ़ 3 बार) | 5 |
| Daily Challenge (आज 10 सवाल सही) — दिन में एक बार | +40 |
| Daily study goal पूरा — दिन में एक बार | +30 |
| Brain Game जीत (दिन में अधिकतम 20 XP) | 10 |
...
| समस्या | हल |
|---|---|
| सफ़ेद/खाली पेज | `index.html` root में है? Pages चालू है? 1–2 मिनट रुककर refresh करो |
| "Website का data लोड नहीं हो पाया" | Pages के लिंक से खोलो (file:// से नहीं); `data/subjects.json` upload हुई है? |
| Chapter में "0 प्रश्न" | उस chapter की JSON में प्रश्न जोड़े? **सेटिंग → Data Check** देखो |
...
Bottom menu के **📓 नोट्स·PYQ** बटन से नए sections खुलते हैं (My Notes, PYQ, Mixed Practice, Notes Game, Fun Vault) और ऊपर 🔊 Sound बटन है। ये Chapter Quiz से अलग हैं और अभी खाली हैं (Content आप जोड़ोगे)।
...
| नई फ़ाइल / folder | काम |
|---|---|
| `js/extras-core.js`, `js/extras-quiz.js`, `js/notes.js`, `js/pyq.js`, `js/practice.js`, `js/notes-game.js`, `js/vault.js`, `js/sound.js` | नए sections का code |
